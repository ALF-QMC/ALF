...
module upgrade_mod
   ...
   contains
      ...
      Subroutine Upgrade_freeproj(GR,N_op,PHASE,Hs_new,i_wlk)
        ...
        st_r = 0.d0
        do nu_c = 1, nu_spin
            nsigma_new%f(1,1)  = field_list(nu_c) 
            ed_r = st_r + nsigma_new%px0(1,1)
            if ( (rand_nu .ge. st_r ) .and. (rand_nu .lt. ed_r )  ) then
                n_prop = nu_c
                exit
            endif
            st_r = st_r + nsigma_new%px0(1,1)
        enddo
        !--------------------------------------------------!
        nsigma_new%f(1,1)  = field_list(n_prop)

        S0_ratio           = ham%S0(n_op,1,dble(field_list(n_prop)))
        ...
        !! update weight
        abs_wk  (i_wlk) = abs_wk  (i_wlk)*sqrt(ratio_o*conjg(ratio_o))
      ...
      End Subroutine Upgrade_freeproj

      Subroutine Upgrade2(GR,N_op,PHASE,Hs_new,i_wlk)
        ...
        if( real(ratio_o,kind(0.d0)) .lt. 0.d0 ) then
            weight_k(i_wlk) = 0.d0
        else
            !! update weight
            weight_k(i_wlk) = weight_k(i_wlk)*real(ratio_o,kind(0.d0))
      ...
      Subroutine Upgrade_mc(GR,n_op,nt,PHASE, Hs_new, i_wlk )
        ...
        Ratiotot = Product(Ratio)
        nf = 1
        Ratiotot = (Ratiotot**dble(N_SUN)) * nsigma_new%Gama(1,1)/nsigma_bp(i_wlk)%Gama(n_op,nt)

        Weight   = S0_ratio * abs( real(Phase * Ratiotot, kind=Kind(0.d0))/real(Phase,kind=Kind(0.d0)) )
