...
# -----------------------------------------
def sweep_v1_v2(t1, t2, V1_list, V2_list, mode="append", **kwargs):
    ...
    # 读取已完成的 (V1, V2) 组合，避免重复计算
    done_pairs = set()
    if os.path.exists(csv_file) and mode == "append":
        with open(csv_file, "r") as f:
            reader = csv.DictReader(f)
            for row in reader:
                if float(row["t1"]) == t1 and float(row["t2"]) == t2:
                    done_pairs.add( (float(row["V1"]), float(row["V2"])) )

    for V2 in V2_list:
        for V1 in V1_list:
            if (V1, V2) in done_pairs:
                print(f"=== Skipping V1={V1}, V2={V2} (already in CSV) ===")
                continue

            print(f"\n=== Optimizing for V1={V1:.3f}, V2={V2:.3f} ===")
            M_best = run_trials_with_refinement(t1=t1, t2=t2, V1=V1, V2=V2, **kwargs)
            G_best = compute_G(M_best)
            nn_bonds, nnn_bonds, nn_dirs, nnn_dirs = get_bond_lists(Lx, Ly)
            energy = energy_wick(G_best, nn_bonds, nnn_bonds, nn_dirs, V1, V2, t1, t2)
            staggered_order = compute_staggered_order(G_best, sublattices)

            key = f"t1_{t1:.3f}_V1_{V1:.3f}_V2_{V2:.3f}"
...
if __name__ == '__main__':
    ...
    sweep_v1_v2(
        t1=1.0,
        t2=0.5,
        V1_list=np.linspace(0, 2, 2),
        V2_list=np.linspace(0, 2, 2),
        Lx=4,
        Ly=4,
        rough_iter=300,
        refine_iter=1000,
