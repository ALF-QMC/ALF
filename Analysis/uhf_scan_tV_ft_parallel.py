...
def find_mu(eigvals, Nelec, T, tol=1e-10):
    mu_low = eigvals[0] - 10 * T
    mu_high = eigvals[-1] + 10 * T
    for _ in range(1000):
        mu = 0.5 * (mu_low + mu_high)
        f_occ = 1.0 / (1.0 + np.exp((eigvals - mu) / T))
        Neff = np.sum(f_occ)
        if abs(Neff - Nelec) < tol:
            return mu
        if Neff > Nelec:
            mu_high = mu
        else:
...
def total_density_matrix(H, Nelec, T=0.0):
    eigvals, eigvecs = np.linalg.eigh(H)
    if T <= 1e-4:
        F_occ = np.zeros_like(eigvals)
        F_occ[:Nelec] = 1.0
    else:
        mu = find_mu(eigvals, Nelec, T)
        F_occ = 1.0 / (1.0 + np.exp((eigvals - mu) / T))
    n_matrix = eigvecs @ np.diag(F_occ) @ eigvecs.T.conj()
    slater_matrix = eigvecs[:, :Nelec]
...
def generate_initial_state(state_type, Lx, Ly, rng, amp_params=(0.3,0.7), noise_level=0.05):
    Nsite = 2 * Lx * Ly
...
def run_optimization_with_ninit(n_init, Lx, Ly, t1, t2p, V1, V2, Nelec, maxiter=500, pbc=True, alpha=0.5, T=0.0):
    ...
    for i in range(maxiter):
        ...
        n_matrix, slater_matrix = total_density_matrix(H, Nelec, T)
    ...
    interaction_total = interaction1 + interaction2 - const_term

    eigvals = np.linalg.eigvalsh(H)

    if T < 1e-4:
        # 零温总能量
        energy_kin = np.trace(H @ n_matrix).real
        energy = energy_kin - interaction_total
    else:
        # 有限温自由能
        beta = 1.0 / T
        mu = find_mu(eigvals, Nelec, T)
        F_kin = - (1.0 / beta) * np.sum(np.log(1.0 + np.exp(-beta * (eigvals - mu))))
        energy = F_kin - interaction_total
...
def worker_task(args):
    v1, v2, T, params_base, initial_state_configs, random_keys, Lx, Ly = args
    group_key = f"V1_{v1:.3f}_V2_{v2:.3f}_T_{T:.4f}"
    ...
    for state_type, amp_list in initial_state_configs:
        ...
        for key, amp in zip(keys, amps):
            ...
            n_mean, energy, n_expect, slater = run_optimization_with_ninit(
                ...
                V1=v1, V2=v2, Nelec=params_base['Nelec'],
                maxiter=params_base['maxiter'], pbc=params_base['pbc'], T=T
            )
...
if __name__ == "__main__":
    ...
    Lx, Ly = 4, 4
    params_base = dict(t1=1.0, t2p=0.5, Nelec=16, maxiter=2000, pbc=True)

    #V1_list = np.linspace(0.1, 2, 10)
    #V1_list = np.linspace(0.01, 4, 799)
    #V1_list = np.linspace(1.30, 1.31, 21)
    #V1_list = np.linspace(1.530, 1.541, 21)
    V1_list = [1.50]
    V2_list = [0.00]
    #T_list = [0.01]
