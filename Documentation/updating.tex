...
\begin{table}[h]
	\begin{tabular}{@{} p{0.45\columnwidth}  p{0.35\columnwidth}  p{0.15\columnwidth}  @{}}
		\toprule
		Updating schemes                          &  Method & Section \\
		\midrule
		Sequential space-time discrete fields   &      Metropolis, spin flip   & \ref{sec:sequential},\ref{sec:S0}  \\
		Sequential space-time continuous fields &      Metropolis, box move   & \ref{sec:sequential_box} \\
		Sequential space-time continuous fields &      Metropolis assisted Langevin   & \ref{sec:sequential_mala} \\
		Sequential time, global space           &      Metropolis, User defined            & \ref{sec:global_space} \\
		Sequential time, global space           &      Metropolis assisted Langevin   & \ref{sec:global_space_mala} \\
		Sequential space-time, global space     &      Combinations of moves          & \ref{sec:sequential_combination} \\
		Global space-time, discrete/continuous &      User defined, Metropolis            & \ref{sec:global_space_time_M} \\
		Global space-time, continuous          &      User defined, Metropolis assisted Langevin            & \ref{sec:global_space_time_MALA} \\
		Global space-time, continuous          &      Parallel tempering         & \ref{Parallel_tempering.sec} \\
		Global space-time, continuous          &      Langevin dynamics         & \ref{sec:langevin} \\
		Global space-time, continuous          &      Hybrid Monte Carlo                & \ref{sec:hmc} \\
		\bottomrule
	\end{tabular}
	\caption{Summary of possible updating schemes. Aside form the parallel tempering schemes, the user can test and compare all these schemes using  our implementation of the Hubbard model. Here we 
	recommend using a one-dimensional lattice with open boundary conditions and a Hubbard-Stratonovitch  decoupling that  couples to the magnetization. As mention in Sec.~\ref{sec:langevin}, this choice 
	guarantees the absence of zero in the fermion determinant such that all  updating schemes will work out of the box and can be compared. }
	\label{table:Updating_schemes}
\end{table}
...
% 		\texttt{Sequential\_MALA}   & Delta\_t\_MALA\_sequential, Max\_Force\_MALA\_sequential    &  S0, Ham\_Langevin\_HMC\_S0\_single \\
...
\paragraph{Box distribution} 
\label{sec:sequential_box}
The first one is the default and consists in proposing a new value for the field according to a box distribution of width \texttt{Amplitude} centered around the origin.  This updating scheme is symmetric, and the move is accepted with probability $P(C \rightarrow C') =  \text{min}  \left( 1, \frac{ W(C')}{W(C)} \right)$. This is the default updating scheme for continuous fields. 

\paragraph{Metropolis assisted Langevin algorithm (MALA)} \label{sec:sequential_mala}
...
\end{equation}
The user can control the time step $\delta t$ by setting the variable \texttt{Delta\_t\_MALA\_sequential} in the input file.  
We use an adaptive time-step  capped by the variable:\texttt{Max\_Force\_MALA\_sequential}.  Let $F_{max}$  be the maximum value of the force at any give simulation time, then the effective time step is given by $\delta t_{eff} = \delta_t * \text{min}(1, \frac{\texttt{Max\_Force\_MALA\_sequential}}{F_{max}})$.
...
Sequential_MALA            = .T.   ! MALA sequential moves for continuous fields
...
\paragraph{User-defined global moves on a single time slice: Metropolis}
...
Subroutine Overide_global_tau_sampling_parameters(Nt_sequential_start,Nt_sequential_end,&
	...
	Integer, Intent(INOUT) :: N_Global_tau, Nt_sequential_start, Nt_sequential_end, &
                                        & N_Global_tau_MALA
...
%------------------------------------------------------------
\paragraph{User defined global moves on a single time slice: MALA}
\label{sec:global_space_mala}
...
subroutine Global_MALA_move_tau(Flip_list, Flip_length,ntau)
    ...
    Integer, INTENT(OUT) :: Flip_length
    Integer, INTENT(IN)  :: ntau
	!   Fields  nsigma%f(Flip_list(1),ntau) ... 
	!           nsigma%f(Flip_list(Flip_Length),ntau) will be updated.
...
\label{sec:sequential_combination}
...
......
\end{lstlisting}
Will carry out sequential single spin-flip moves on the first three operators of the operator list using the MALA updating scheme \ref{sec:sequential_mala}, followed by \texttt{N\_Global\_tau=6} global moves on the time slice.
...
\paragraph{MALA} 
\label{sec:global_space_time_MALA}
Generically,  user defined global moves have a poor acceptance rate. For continuous variables, one  can improve this by adopting the 
Metropolis assisted Langevin algorithm (MALA).   
Since the move is user defined, one has to provide a function that tags the fields that be updated during the global MALA move.
This function is specified in the  Hamiltonian, and takes the following  form: 
\begin{lstlisting}[style=fortran]   
Subroutine Global_MALA_move(Flip_list)
	Integer                   , INTENT(OUT) :: Flip_list(size(Op_V,1),Ltrot)
	....
	Flip_list(n,nt) = 1  ! Field will be updated
	Flip_list(n,nt) = 0  ! Field will not be updated
	....
end Subroutine Global_MALA_move
\end{lstlisting}
The default implementation sets all entries of the \texttt{Flip\_list} to unity. 

To activate this type of move, one  needs to modify the following in the parameter file:
...
Sequential              = .T.  ! Flag for sequential moves
.......
Global_MALA_moves       = .T.  ! Allows for global MALA updates
N_Global_MALA_sweeps    = 1    ! Number of global MALA sweeps
Delta_t_MALA_global     = 0.1  ! Time step for global MALA updates
Max_Force_MALA_global   = 5.d0 ! Maximum force for global MALA updates
......
/
\end{lstlisting}
% 
