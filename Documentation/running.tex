...
\begin{itemize}
	\item \verb|convert_scal.out| and \verb|convert_latt.out|: Writing the contents of a \verb|_scal| observable or a lattice type observables (so both \verb|_eq|, and \verb|_tau|) to an HDF5 file. E.g. writing bing from \verb|some_obs_scal|, \verb|some_obs_eq| and \verb|some_obs_tau| to data.h5 is done by executing.
	\vspace{-\baselineskip}
	\begin{lstlisting}[style=bash]
...
$ALF_DIR/Analysis/convert_latt.out some_obs_tau data.h5
	...
	\begin{itemize}
		\item Automatically combines file \verb|parameters| and the Hamiltonian used in the simulation to determine simulation parameters and write them into HDF5 file.
		\item Calls \verb|convert_scal.out| and \verb|convert_latt.out| for all observables.
	\end{itemize}
...
find . -name "$ref" -execdir ${ALF_DIR}/Analysis/copy_parameters.py \;
	...
	\item \verb|convert_batch.sh|: Executes the command above. Can also be used to convert the bins without copying parameters.
\end{itemize}
