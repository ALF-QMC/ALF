...
The source code of ALF~\ALFver{} is available at \url{https://github.com/ALF-QMC/ALF/tree/\ALFbranch/} and can be cloned with git or downloaded from the repository (make sure to choose the appropriate release).

A Python interface, \textbf{pyALF}, is also available and can be found, together with a number of Jupyter notebooks exploring the interface's capabilities, at \url{https://github.com/ALF-QMC/pyALF/tree/\pyALFbranch/}, with the documentation \url{https://alf.physik.uni-wuerzburg.de/pyalf-doc/}. This interface facilitates setting up simple runs and is ideal for setting benchmarks and getting acquainted with ALF. Some of pyALF's notebooks form the core of the introductory part of the \href{https://github.com/ALF-QMC/ALF_Tutorial_and_Presentations/blob/master/Tutorial-ALF-\tutALFver/}{ALF Tutorial}, where pyALF's usage is described in more detail.
...
The aim of this section is to provide a fruitful and stress-free first contact with the package. Ideally, it should be possible to copy and paste the instructions below to a Debian/Ubuntu-based Linux shell without further thought\footnote{For other systems and distributions see the package's  \href{https://github.com/ALF-QMC/ALF/blob/\ALFbranch/README.md}{README}.}. Explanations and further options and details are found in the remaining sections and in the \href{https://github.com/ALF-QMC/ALF_Tutorial_and_Presentations/blob/master/Tutorial-ALF-\tutALFver/}{Tutorial}.
...
git clone -b #\ALFbranch#https://github.com/ALF-QMC/ALF.git
cd ALF
...
%
%-------------------------------------------------------------------------------------
\subsection{Convert plain text bins to HDF5} \label{sec:convert_to_hdf5}
%-------------------------------------------------------------------------------------
%
The Analysis folder also contains programs for converting plain text bins to HDF5, writing the bins from files ending in \verb|_scal|, \verb|_eq|, or \verb|_tau| to \verb|data.h5|, thereby reducing file sizes and increasing compatibility with pyALF.

There are a number of scripts and program:
\begin{itemize}
	\item \verb|convert_scal.out|, \verb|convert_latt.out|, \verb|convert_local.out|: Writing the contents of a \verb|_scal| observable, a lattice type observables (that is \verb|_eq|, and \verb|_tau|), or a local observable (that is \verb|_local|, and \verb|_localtau|) to an HDF5 file. E.g. writing bins from \verb|some_obs_scal|, \verb|some_obs_eq|, \verb|some_obs_tau|, \verb|some_obs_local| and \verb|some_obs_localtau| to data.h5 is done by executing.
	\vspace{-\baselineskip}
	\begin{lstlisting}[style=bash]

$ALF_DIR/Analysis/convert_scal.out some_obs_scal data.h5
$ALF_DIR/Analysis/convert_latt.out some_obs_eq data.h5
$ALF_DIR/Analysis/convert_latt.out some_obs_tau data.h5
$ALF_DIR/Analysis/convert_local.out some_obs_local data.h5
$ALF_DIR/Analysis/convert_local.out some_obs_localtau data.h5
	\end{lstlisting}
	\item \verb|convert_bins.py|: 
	\begin{itemize}
		\item Automatically combines file \verb|parameters| and the Hamiltonian used in the simulation to determine Hamiltonian-specific simulation parameters and write them into HDF5 file.
		\item Calls \verb|convert_scal.out|, \verb|convert_latt.out|, and \verb|convert_local.out| for all observables.
		\item Moves the original plain text bins to a subdirectory \verb|old_bins| or optionally removes them.
	\end{itemize}
	You can also get a help text by executing:
	\begin{lstlisting}[style=bash]
$ALF_DIR/Analysis/convert_bins.py --help
	\end{lstlisting}
	One can automatically convert a big number of simulations by executing,
	\begin{lstlisting}[style=bash]
find . -name "$ref" -execdir ${ALF_DIR}/Analysis/copy_parameters.py \;
	\end{lstlisting}
	where \verb|$ref| has to be the name of an observable produced by the simulation, e.g. \verb|Ener_scal|.
	\item \verb|convert_batch.sh|: Executes the command above.
\end{itemize}
