...
\begin{breakablealgorithm}
	...
	\begin{algorithmic}[1]
		...
		\State{\textbf{call} Alloc\_Ham}\Comment{Select Hamiltonian based on \texttt{ham\_name} in \texttt{parameters}}
		\State{\textbf{call} ham\%Ham\_Set}\Comment{Set the Hamiltonian and the lattice}
		...
		\State{\textbf{call} ham\%Obser} \Comment{\textsc{Measure} the equal-time observables}
		\EndIf
		\EndFor
		\vspace{1.5ex}
		...
		\State{\textbf{call} ham\%Pr\_obs} \Comment{Calculate and write to disk measurement averages for current bin}
