...
For each pair of indices $(k,s)$, we store the following parameters in a \path{Operator} variable:
\begin{itemize}
...
We have adopted the following name convention. 
...
the routine \texttt{Print\_bin\_Latt}  attaches the suffix \_eq to \texttt{Obs\%File\_Latt}.  For  time displaced correlation functions we use the suffix \_tau. 
