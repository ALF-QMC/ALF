...
ALF 2.1 introduces a change that is \textbf{not backwards compatible} with ALF~2.0:
% present in 2.0 i) lattice information is now embedded in the observable types; and ii)
Hamiltonians are now written as submodules of the module \path{Hamiltonian_main}. Sec.~\ref{sec:hamiltonian} details the new implementation and should ease adjusting existing code to ALF~2.1, which takes 2 steps:
\begin{itemize}[itemsep=0pt,topsep=0pt]
	\item restructuring your Hamiltonian as a submodule,
	\item adding your Hamiltonian name to the file \texttt{Prog/Hamiltonians.list}.
%	\item include the parameter \texttt{Mc\_step\_weight} in \texttt{ObserT} and \texttt{Obser} routines, % in 2.0
%	\item define subroutine \texttt{Ham\_Langevin\_HMC\_S0(Forces\_0)} in \texttt{Prog/Hamiltonians.list}, % in 2.0
%	\item adjust calls to \texttt{Obser\_Latt\_make} to conform to the subroutine's new interface. % in 2.0
%	\item ... nsigma
\end{itemize}
