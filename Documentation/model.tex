...
Note that the chemical potential term is already included in the definition of the one-body term ${\mathcal{\hat{H}}_{T}}$ (\ref{eqn:general_ham_t}) of the general Hamiltonian.  
...
Next, we present a number of updating strategies  (Sec.~\ref{sec:updating}). 
...
\begin{itemize}
\item  We will discretize the imaginary time propagation: $\beta = \Delta \tau L_{\text{Trotter}} $.   Generically this introduces a systematic Trotter error of order $(\Delta \tau)^2$  \cite{Fye86}. 
...
The negative sign problem is an issue average sign is a ratio of two partition functions such that one can argue  that 
\begin{equation}
...
\end{equation}   
Two comments are in order. i)  The presence of  a sign problem invariably leads to an exponential increase of CPU time as a function of the Euclidean volume. ii)  $\Delta$ is formulation dependent.  
