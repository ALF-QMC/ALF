...
The first column of the  \texttt{Green}  file is a placeholder for post-processing. The last three columns   correspond to $\omega, \Re G(\omega) ,   \Im G(\omega) $. The above serves as a smoothing procedure of the spectral function. In particular  if $\delta = 0^+$  then  $\Im G(\omega) = A(\omega) $.
...
and, in the Lehmann representation, 
 \begin{equation}
   A_p(k, \omega) = \frac{ \pi}{Z} \sum_{n,m} e^{-\beta E_m } \left( 1 + e^{-\beta \omega}\right) | \langle n | c_n | m  \rangle |^{2} \delta \left( E_m - E_n - \omega \right)  .
\end{equation}  
...
\end{equation}
The above follows from  the identity $\Theta(t) =  \frac{i}{2\pi} \int d \omega'  \frac{e^{-i \omega' t}}{\omega' + i 0^{+}} $.
...
with 
\begin{equation*}
   \chi(q,\omega)  = i \int \d t \Theta(t)  e^{i \omega t} \langle \big[ \hat{S}(q,t), \hat{S}(-q,0) \big] \rangle .
\end{equation*}
...
\begin{equation}
	S(q,\omega) = \frac{A_{ph}(\omega)}{1 + e^{-\beta \omega} } .
\end{equation}
...
\subsection{ Integration  over  the  spectral  function: QFI and  $\frac{\partial I}{\partial V}$ } 
...
Another  example is  the  quantum Fisher  information \cite{Hauke16} (QFI).  In the particle-hole channel,  this  quantity  reads: 
\begin{equation}
...
\end{equation}
The  last  step follows  for  Eq.~\ref{eqn:A_ph_def}.    
In the default implementation  the  QFI is computed for  the particle-hole channel, and  the results are written in the file \texttt{QFI.dat}.  The first column of this file corresponds to the inverse temperature, $1/\alpha$, the second column to the QFI, and the third column to the error bar.  The function $F$ is specified in the module \texttt{maxent\_wrapper\_mod.F90} in the  \texttt{Analysis}   directory.
This function is  passed to the stochastic MaxEnt routine in the called from the wrapper \texttt{Max\_SAC.F90}.  The user can modify the function $F$ to compute other quantities and specify the output of the file name. 

For the particle channel the  default corresponds  to the calculation of  $\left. \frac{\partial I}{\partial V}\right|_{V=0}$, given by  the 
Meier-Wingreen formula \cite{Meir92}:
\begin{equation}
 \left. \frac{\partial I}{\partial V}\right|_{V=0} = - \int d \omega A(\omega) \frac{\partial f(\beta \omega)}{\partial \omega} 
\end{equation}
with $f(x) = 1/(1+e^x)$ the Fermi function.  The  evaluation of this quantity is based on the work of Ref.~\cite{Karrasch10} where the 
Fermi function is approximated by: 
\begin{equation}
  f(x) - \frac{1}{2} = - \sum_{\alpha =1 }^{N/2} \left( \frac{R_\alpha}{x - i/ b_\alpha} + \frac{R_\alpha}{x + i /b_\alpha} \right).
\end{equation} 
For this parametrization, it turns out  that truncating the sum to ten terms provides a perfect approximation  for $|x|<40$.  
In  the MaxEnt wrapper \texttt{Max\_SAC.F90}  the call to the routine \texttt{Set\_Ra\_ba(N)} sets the parameters $R_\alpha$ and $b_\alpha$ for the approximation of the Fermi function (see Ref.~\cite{Karrasch10}).  The default value of $N$ is set to ten  resulting in the aforementioned truncation, but the user can change it to improve the approximation for larger values of $|x|$.   With the above,  one  can estimate:
\begin{equation}
   \left. \frac{\partial I}{\partial V}\right|_{V=0}  = \int d \omega A(\omega) \left[ -\frac{2}{\beta} 
   \sum_{\alpha>0}  R_\alpha \Re \frac{1}{(i/(\beta b_\alpha) - \omega)^2} \right] 
\end{equation}
The results are written in the file \texttt{dIdV.dat} with the first column corresponding to the temperature, the second column to $\left. \frac{\partial I}{\partial V}\right|_{V=0}$, and the third column to the error bar.
...
\href{https://github.com/ALF-QMC/pyALF/blob/\pyALFbranch/Scripts/Hubbard_1D.py}{\texttt{Hubbard\_1D.py}}, and   the spectral function plots with the bash script \href{https://github.com/ALF-QMC/pyALF/blob/\pyALFbranch/Scripts/Spectral.sh}{\texttt{Spectral.sh}}. 
...
(a,d) Dynamical charge structure factor, (b,e) single particle spectral function and (c,f) dynamical spin structure factor. 
 ...
 \href{https://github.com/ALF-QMC/pyALF/blob/\pyALFbranch/Scripts/Hubbard_1D.py}
