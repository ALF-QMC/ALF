...
%	\end{center}
\includegraphics[width=1.0\textwidth]{Autocorrel_Error.pdf}
	\caption{\label{fig_autocorr}
		Autocorrelation (left) and scaling of the error with the bin size (right). We use a Hubbard model on the square lattice in the $M_z$ decoupling (see Sec.~\ref{sec:walk1.1})  $U/t=4$, $\beta t = 6$ on a $ 6 \times 6$ lattice.  The original bin contains only one sweep and we calculated around 1 million bins on a single core. We analyze the spin-spin-correlation at $(\pi,\pi)$  and clearly detect different autocorrelation times and therefore required bin sizes for the $xy$-plane compared to the $z$-direction.  The  improved estimator, corresponds to the sum of the spin correlations in the $x$,$y$, and $z$-directions, and as apparent as argued in the text, has very much smaller  autocorrelation times. 
\FFAcomment{Figures need updating.  Labels (a) and (b) etc.... } }
\end{figure}

In Fig.~\ref{fig_autocorr} (left) we present the autocorrelation functions for the spin-spin-correlation at $(\pi,\pi)$ both in the $xy$ and the $z$  spin-directions. The Hubbard model  has an $SU(2)$-spin symmetry. However, we chose a HS field that couples to the z-component of the magnetization,  $M_z$,  such that each configuration breaks the symmetry. Of course after Monte Carlo averaging one expects restoration of this symmetry. The model, on bipartite  lattices,  shows spontaneous spin-symmetry breaking at $T=0$ and in the thermodynamic limit.  At finite temperatures, and within the so-called renormalized classical regime,  quantum antiferromagnetics have a length scale  that  diverges  exponentially  with decreasing temperatures \cite{Chakravarty88}.     
The parameter set chosen in Fig.~\ref{fig_autocorr}  is non-trivial in the sense that it places the Hubbard model in this renormalized classical regime where the correlation length is substantial.  Fig.~\ref{fig_autocorr} (left) clearly shows a very short autocorrelation time for the $xy$-plane whereas we detect a considerably longer  autocorrelation times  for the $z$-direction.  This is a direct consequence {\it long} magnetic length scale and the chosen decoupling.
The physical reason for the long autocorrelation time  corresponds to  the restoration of the SU(2) spin symmetry.    This insight can be used to define an improved estimator for the spin correlation function  that is  $SU(2)$ symmetric. Thereby, global spin rotations are no longer an issue and this improved estimator  shows the shortest autocorrelation time as seen in Fig.~\ref{fig_autocorr} (b). Other ways to tackle large autocorrelation can be global updates or parallel tempering.
