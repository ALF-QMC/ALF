...
\newcommand{\FFAcomment}[1]{{\color{red} #1}}
...
are QUEST \cite{QUEST}, TRIQS \cite{TRIQS}, w2dynamics and  iQist \cite{iQIST}.
...
\mycomment{ FFA What follows is too much information for the purpose of the documentation. I hence propose to leave it as is.}

\st{We have a public mailing list that is open to anyone to join the discussion.
The organizational structure of our project is simple. Anybody can use the code
and propose changes to it, but the sole decision of which changes will get accepted into a release of the ALF code and
which direction the project takes rests with Fakher Assaad.
Nevertheless it is beneficial to be a part of our community. 
We will try to connect users that try to tackle problems with the ALF package.
We think that it is beneficial to users that they can benefit from the experience of others
and that we hope to form a community of researchers from different fields working on different problems.
To that end we plan to organize workshops and community meetings that everybody is able to attend.
Using gitlab we have the technical means to trace the contributions of everybody,
to make sure that every contributor is listed on our project website
or in the case of greater contributions up to authorship on a relevant contribution.
Organizing workshops, hosting the servers and investing the time takes some effort
but we are committed to invest that to grow a community.}
%
%------------------------------------------------------------
...
\mycomment{FFA:  As we will detail below, the definition of the above Hamiltonian allows  to tackle  several  non-trivial models and phenomena.  There are however a number of model Hamiltonians that cannot be simulated with the ALF.  Since we have opted for discrete  fields,   the electron-phonon interaction is not included. Furthermore, continuous HS transformations, that turn out to be extremely useful to include long ranged Coulomb interactions \cite{Brower12,Ulybyshev2013,Hohenadler14},   are not accessible  in the present form of the package. \footnote{Note however that  one can readily add  short ranged interactions  by including terms such as $ (\hat{n}_i + \hat{n}_j - 2 )^2$. } In many cases such as in $^3$He,  three  or more  body interactions  should be included to capture  relevant   exchange mechanisms \cite{Roger83,JWerner14a}. These higher order processes are   not captured in the ALF since it is \textit{limited}  to two-body interactions.  Finally, the formulation of the Hamiltonian, has an  explicit global $U(1)$ symmetry reflecting that particle number conservation.  Hence using the ALF for  a given model implies the existence of a canonical transformation where  a particle number is conserved.    Imaginary time dependent Hamiltonians, required to compute Rennyi entropies and entanglement spectra   \cite{Broecker14,Assaad15, Broecker16} are not yet  included the ALF.   One should also mention that auxiliary field QMC simulations are Hamiltonian based such that retarded interactions  are not included in the ALF.    For this set of problems, CT-INT type approaches are the method of choice \cite{Assaad07,Werner07,Assaad14_rev}.     The above {\it short comings }  define a set of  future directions that will be discussed in the concluding part of this documentation.  }
%
%------------------------------------------------------------
\subsection{Outline}
%------------------------------------------------------------
...
%
%Note that 
for a given number of stabilization steps along the imaginary time axis [in the notation of Eq.~(\ref{eqn:green_stable}) this number is $L_{\text{Trotter}}/\texttt{NWrap}$], the precision will be largely invariant of the system size $N$,
%However, 
whereas
...
% 
% \mycomment{
% To summarize, the algorithm is stablized by re-computing the Green function matrix from scratch on a subset of time slices. 
% While the stabilization will increase the prefactor of the computational effort, it will not change its exponents. That is the time complexity of  $\mathcal{O}(\beta N^{3})$ of the algorithm remains.
% Note that for a given number of stabilization steps along the imaginary time axis [in the notation of Eq.~(\ref{eqn:green_stable}) this number is $L_{\text{Trotter}}/\texttt{NWrap}$], the precision will be largely invariant of the system size $N$. 
% However, with increasing inverse temperature $\beta$ the number of stabilization steps often has to be increased to maintain a given precision.
% }
%
...
%we refer the reader to the pedagogical introduction in chapter 1.3.5 of Krauth\cite{Krauth2006}
%and the more specialized literature by Geyer\cite{Geyer1992} and chapter 6.3 of Neal\cite{neal1993}.
...
measurements and  a finite variance.  \mycomment{
In this subsection we will give examples of the issues that a user will have to look out for while 
using a Monte Carlo code. Those effects are part of the common lore of the field
and we can only touch on them briefly  in this text.
For a deeper understanding of the inherent issues of Markov chain Monte Carlo methods 
we refer the reader to the pedagogical introduction in chapter 1.3.5 of Krauth\cite{Krauth2006}
and the more specialized literature by Geyer\cite{Geyer1992} and chapter 6.3 of Neal\cite{neal1993}. 
In general, one distinguishes local from global updates. As the name suggest, the local update is only a small change of the configuration, e.g. a single spin flip on one of the $L_{\mathrm{Trotter}}(M_I+M_V)$ field entries (see Sec.~\ref{sec:updating}), whereas a global update changes a significant part of the configuration. The default update scheme of the implementation at hand are local updates such that a minimum amount of moves is required to generate a independent configuration. The associated time-scale is called autocorrelation time $T_\mathrm{auto}$, and is generically dependent upon the choice of the observables. }
%The default updating scheme consists of local moves which change (upon acceptance) only one  entry of $L_{\mathrm{Trotter}}(M_I+M_V)$  fields 
%To generate  an independent configuration $C$,   one has to visit at least each field  once. 
...
\mycomment{
%------------------------------------------------------------
\subsubsection{The Jackknife resampling method}
%------------------------------------------------------------
For each observable $\hat{A}, \hat{B},\hat{C} \cdots$ the Monte Carlo program computes a data set of $N_{\text{Bin}}$ (ideally) independent values where for each observable the measurements belong to the same but usually unknown statistical distribution. This calls for a numerical statistical analysis. In the general case, we would like to evaluate a function of expectation values, $f(\langle \hat{A} \rangle, \langle \hat{B} \rangle, \langle \hat{C} \rangle  \cdots)$ --
...
A method for the statistical analysis of a given function $f$ which properly handles error propagation and correlations among the observables is the Jackknife method, which is, like the related Bootstrap method, a resampling scheme \cite{efron1981}.
...
\subsubsection{An explicit example of error estimation}
\label{sec:autocorr}
\mycomment{
In the following we use one of our examples, the Hubbard model on a square lattice in the $M_z$ Hubbard-Stratonovich decoupling (see Sec.~\ref{sec:walk1.1}), to show explicitly how to estimate errors.  We will equally show that  there is no unique autocorrelation time. In fact, different observables within the same run can have different autocorrelation times  and of course, this time scale depends on the  parameter choice.  Hence, the user has to check the autocorrelation of individual observables for each simulation!  Typical regions of the phase diagram that require special attention are critical points  where length scales diverge.  

To determine the autocorrelation time, we calculate the correlation function
\begin{equation}
	Auto_O(t_{\textrm{QMC}})=\sum_{i=0}^{N_{\textrm{Bin}}-t_{\textrm{QMC}}}\frac{(O_i-\left\langle O\right\rangle )(O_{i+t_{\textrm{QMC}}}-\left\langle O\right\rangle )}{(O_i-\left\langle O\right\rangle )(O_{i}-\left\langle O\right\rangle )}\, ,
\end{equation}
where $O_i$ refers to the Monte Carlo estimate of the observable $O$ in the $i^{\text{th}}$ bin. This function typically shows an exponential decay and the decay rate defines the autocorrelation time.

In Fig.~\ref{fig_autocorr} (left) we present the autocorrelation functions for the spin-spin-correlation at $(\pi,\pi)$ both in the $xy$ and the $z$  spin-directions. The Hubbard model on a square lattice is $SU(2)$ symmetric. However, we chose a HS field that couples to the z-component of the magnetization,  $M_z$,  such that each configuration breaks the symmetry. Of course after Monte Carlo averaging one expects restoration of this symmetry. This model shows spontaneous symmetry breaking at $T=0$ and in the thermodynamic limit.  At finite temperatures, and within the so-called renormalized classical regime,  quantum antiferromagnetics have a length scale  that  diverges  exponentially  with decreasing temperatures \cite{Chakravarty88}.  The parameter set chosen in Fig.~\ref{fig_autocorr}  is non-trivial in the sense that it places the Hubbard model in this renormalized classical regime.   Fig.~\ref{fig_autocorr} (left) clearly shows a very short autocorrelation time for the $xy$-plane whereas we detect a considerably longer  autocorrelation times  for the $z$-direction.  This is a direct consequence {\it long} magnetic length scale and the chosen decoupling.
From the obtained time series of Monte Carlo samples we need to obtain estimates on the mean and the variance of the data.
A simple method that we will describe in this tutorial is the rebinning method, also known in the literature as rebatching. For a comparison to other methods of estimating the autocorrelation time we refer the reader to the literature \cite{Thompson2010, Geyer1992, neal1993}.
A reliable error analysis requires independent bins, otherwise the error is typically underestimated. This can also be seen in Fig.~\ref{fig_autocorr} (right). Here we systematically increase the effective bin size by rebinning in which a given number of bins (\path{N_rebin}) are recombined into new bins. If the bin size is smaller than the autocorrelation time, the estimated error is to small, once the bin is larger than the autocorrelation time, the error converged to the correct estimate.

For the analysis, the user can provide a finite value for \path{N_auto} to trigger the analysis of the autocorrelation time. The calculations are quite time consuming and require many Monte Carlo bins, hence the default value is 0 if unspecified. To produce Fig.~\ref{fig_autocorr}, we used $\textrm{\path{N_auto}}=500$ and a total of approximately 1 million bins. \\ 
\FFAcomment{@Johannes: In which branch did you include this $N_{Auto}$ tag? Do we want to include it in this release or in another release.} \\
I have uploaded the autocorrelation analysis in branch 63-analysis-of-autocorrelation\\
\FFAcomment{@Johannes:  Do you get the same numbers for the XY and ZZ correlations? Can you analyze the autocorrelation time of 
$XX + YY + ZZ$} \\
Yes I do:\\
==> SpinXY\_eqJK <==\\
3.141593     -1.047198\\
1    1        0.70946572        0.00484769       -0.00000000        0.00000000\\
3.141593      0.000000\\
1    1        0.60622624        0.00636953       -0.00000000        0.00000000\\
3.141593      1.047198\\
1    1        0.70946572        0.00484769        0.00000000        0.00000000\\
3.141593      2.094395\\
1    1        1.08323975        0.00596128       -0.00000000        0.00000000\\
3.141593      3.141593 \\
1    1        5.09627233        0.02130724        0.00000000        0.00000000\\
\\
==> SpinZ\_eqJK <==\\
3.141593     -1.047198\\
1    1        0.71069472        0.00071066       -0.00000000        0.00000000\\
3.141593      0.000000\\
1    1        0.60690740        0.00079118        0.00000000        0.00000000\\
3.141593      1.047198\\
1    1        0.71069472        0.00071066        0.00000000        0.00000000\\
3.141593      2.094395\\
1    1        1.08468545        0.00168634       -0.00000000        0.00000000\\
3.141593      3.141593\\
1    1        5.08432433        0.02350014        0.00000000        0.00000000\\
\\
I will analyze the $XX+YY+ZZ$ autocorrelation, I am combining the two correlation functions manually (additional program). I don't know how reasonable this is as we clearly see two very distinct time scales. I expect the new correlation function to be dominated by the longer time scale and we will hopefully have the addition data by the end of the day.\\
\FFAcomment{@Johannes:  This is really a very tricky example.  If you go to lower temperatures and large lattices, then you will never be able to  show that 
$XX = ZZ$. Hence one has to define improved estimators such as $XX + YY + ZZ$  that are explicitly $SU(2)$ invariant} \\ 
I agree. This also means, that we effectively lost ergodicity due to critical slowing down (extremely long autocorrelations in z spin-directions), doesn't it?\\
\FFAcomment{ @ Martin: Will you work on the figures as for the optimization. }
No problem, I would just need the data. I suggest to put them in Documentation/Figures/, like the MPI scaling data.

\begin{figure}[h]
	\begin{center}
		\includegraphics[width=0.49\textwidth]{AutoCorr.pdf}
		\includegraphics[width=0.49\textwidth]{Error.pdf}
	\end{center}
	\caption{\label{fig_autocorr}
	Autocorrelation (left) and scaling of the error with the bin size (right). We use a Hubbard model on the square lattice in the $M_z$ decoupling (see Sec.~\ref{sec:walk1.1})  $U/t=4$, $\beta t = 6$ on a $ 6 \times 6$ lattice.  The original bin contains only one sweep and we calculated around 1 million bins on a single core. We analyze the spin-spin-correlation at $(\pi,\pi)$  and clearly detect different autocorrelation times and therefore required bin sizes for the $xy$-plane compared to the $z$-direction.}
\end{figure}
}
...
% \LeftComment{This loop fills the storage, needed for the first actual Monte Carlo sweep:}
...
% \LeftComment{Loop over bins. The bin defines the unit of Monte Carlo time:}
\For{$n_{\text{bc}}=1$ to $N_{\text{bin}}$}\Comment{\textit{Loop over bins. The bin defines the unit of Monte Carlo time}}
\item[]

% \OneIndentedLeftComment{Loop over sweeps. Each sweep updates twice (sweeping upward and downward in}
% \OneIndentedLeftComment{imaginary time) the whole space-time lattice of auxiliary fields:}
...
% \TwoIndentedLeftComment{Upward sweep:}
\For{$n_{\tau}$ to $L_{\text{Trotter}}$}\Comment{\textit{Upward sweep}}
...
% \ThreeIndentedLeftComment{Measure the equal time observables:}
...
% \TwoIndentedLeftComment{Downward sweep}
...
% \TwoIndentedLeftComment{Measure the time displaced observables:}
...
N_auto  = 100             ! If set to >0 autocorrelation function will be
                          ! computed for scalar and equal time correlation
                          ! functions.                  
/            
\end{lstlisting}
%
...
This  number should be \mycomment{at least} comparable \mycomment{or  lager than} the autocorrelation time. \mycomment{The analysis of the autocorrelation time is triggered by specifying a positive value for \path{N_auto} that is turned off be default ($\textrm{\path{N_auto}}=0$).}
...
Our analysis is based on the Jackknife resampling\cite{efron1981,Assaad02}\mycomment{, which includes proper treatment of the sign.}
...
\begin{table}[h]
   ...
   & where \texttt{Y} stands for \texttt{Green, SpinZ, SpinXY}, and \texttt{Den}.\\
   \texttt{Y\_kx\_ky/g\_kx\_ky} & Time resolved and $\vec{k}$-dependent Jackknife mean and error of \texttt{Y},\\\bottomrule
    \end{tabular}
