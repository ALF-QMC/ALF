...
Of course the ALF package is not the first open source project that aims at the simulation of physical models from solid state physics and the most notable among these projects is certainly the ALPS library \cite{ALPS_2.0}. Other names that come to mind within the realm of the solid state physics
are QUEST \cite{QUEST}, TRIQS \cite{TRIQS}, w2dynamics, iQist \cite{iQIST} and Wien2K.
IQist, TRIQS and w2dynamics focus on approximate solutions via the CT-HYB algorithm within the dynamical mean field approximation. Therefore they cannot cope with large scale lattice simulations in the sense that spatial collective fluctuations are truncated. 
Wien2K is representative for all density functional theory (DFT) based software packages. Systems with large unit cells are certainly amenable to DFT  in the local density approximation. The sheer amount of predictions done with 
DFT methods is astonishing, but there is still a large class of correlated electron systems whose quantum phenomena are not described by the local density approximation.
\mycomment{\textit{M: I think that the following list of deficits of QUEST is too much thrashing. I suggest to abolish the crossed out parts. Maybe we can in the future even win them over to our project ...}}
The QUEST project implements the same algorithm as the ALF project  but \mycomment{\st{it is lacking in two major categories.}} it is currently restricted to the simple Hubbard model
and it does not allow to easily incorporate different models. 
\mycomment{\st{Furthermore,  it does not seem to be actively maintained anymore; the last publication dates from 2013.}} 
\mycomment{\st{An entirely different  approach is}} \mycomment{Finally,} the ALPS library \mycomment{\st{. It}} is actively maintained \mycomment{\st{by a lively group of volunteers as well as paid staff and}} features a whole set of algorithms for \mycomment{strongly correlated quantum lattice models including Monte Carlo, exact diagonalization, and density matrix renormalization group codes} \mycomment{\st{Using the DMRG algorithms they are able to simulate large 1D Systems but they}} but lacks the tools for two dimensional  fermonic  systems offered by the ALF package.

\subsection{Community outreach}
We have decided to put the source code of this tool under the GNU GPL license, an established open source license.
This should be our first, but very important step, towards building a community\cite{Budd2015} around this code.
The goal of this community should be to work together to obtain the best BSS code that is usable for its users.
The project is currently hosted on servers of the university of W\"urzburg and embedded within the research activities of the SFB1170.
Here we have set up a gitlab instance that serves as a central meeting point on where to find the code.
Everybody can sign up here and can receive space for his ALF related projects and share them with others.
To foster the communication between the group members this site serves the gitlab issue tracker and we have a wiki
so that members of our community can collect information they consider useful for the project.
We have set up an E-Mail address for reaching the core developers at alf@physik.uni-wuerzburg.de
and we have a public mailing list that is open to anyone to join the discussion.
...
Assuming for simplicity that \texttt{NWrap} is a multiple of $L_{\text{Trotter}}$ 
\mycomment{The other way round, right?}
, we can write:
...
it has to be noted that even with this stabilization scheme the algorithm preserves the complexity class of $\mathcal{O}(\beta N^3)$ expressed in the physical parameters $\beta$ and the lattice size $N$.
...
%------------------------------------------------------------
\mycomment{
\subsection{Autocorrelation}\label{sec:autocorr}
%------------------------------------------------------------
%
...
and we can only scratch them on the surface in this text.
...
In general, one distinguishes local from global updates. As the name suggest, the local update is only a small change of the configuration, e.g. a single spin flip of one of the $L_{\mathrm{Trotter}}(M_I+M_V)$ field entries, whereas a global update changes a significant part of the configuration. The default update scheme of the implementation at hand are local updates such that a minimum amount of accepted update moves is required to generate a independent configuration. The associated time-scale is called autocorrelation time $T_\mathrm{auto}$.

In the following we use one of our examples, the Hubbard model on a square lattice in the $M_z$ Hubbard-Stratonovich decoupling (see Sec.~\ref{sec:walk1.1}), to show explicitly that there is no unique autocorrelation time. Instead different observables within the same run can suffer from different time scales and of course, the time scale can also change for a different choice of parameters. Therefore the user has to check the autocorrelation of individual observables for each simulation! Typical regions of the phase diagram that require special attention are critical points of phase transitions where length scales are diverging as well as deep inside the ordered regime where the configuration freezes.
...
\end{equation}
where $O_i$ refers to the Monte Carlo estimate of the observable $O$ in the $i$th bin. This function typically shows an exponential decay and the decay rate defines the autocorrelation time.

In Fig.~\ref{fig_autocorr} (left) we present the autocorrelation functions for the spin-spin-correlation at $(\pi,\pi)$ both in the $xy$-plane and the $z$ direction. The Hubbard model on a square lattice is $SU(2)$ symmetric. However, we chose the Hubbard-Stratonovich decoupling in the $M_z$ channel such that each configuration breaks the symmetry which is only restored on average. This model shows spontaneous symmetry by the formation of an anti-ferromagnet and the analyzed observable represents the square of the order parameter. Fig.~\ref{fig_autocorr} (left) clearly shows a very short autocorrelation time for the $xy$-plane whereas we detect a considerably longer time scale for the $z$-direction. This is a direct consequence of both the spontaneous symmetry breaking and the chosen decoupling.
...
\begin{figure}[h]
	...
	Autocorrelation (left) and scaling of the error with the bin size (right). We use a Hubbard model on the square lattice in the $M_z$ decoupling (see Sec.~\ref{sec:walk1.1}) with $LxL=6x6$, $U=4$ and $\beta = 6$. The original bin contains only one sweep and we calculated around 1 million bins on a single core. We analyze the spin-spin-Correlation at $(\pi,\pi)$, which also represents the order parameter of the anti-ferromagnet and clearly detect different autocorrelation times and therefore required bin sizes for the $xy$-plane compared to the $z$-direction.}
\end{figure}
}
%
%------------------------------------------------------------
...
This  number should be \mycomment{at least} comparable to\mycomment{, usually much lager then,} the autocorrelation time. \mycomment{Then analysis of the autocorrelation time is triggered by specifing a positive value for \path{N_auto} that is turned off be default ($\textrm{\path{N_auto}}=0$).}
...
Our analysis is based on the Jackknife resampling\cite{efron1981}\mycomment{, which includes proper treatment of the sign.}
...
\item For the autocorrelation analysis of scalar quantities \texttt{Y}, the output files  \texttt{Y\_eq\_Auto\_Tr\_kx\_ky} have the following formatting:
\begin{alltt}
	do i = 1, N_auto
	   tau(i)/n_rebin   Auto_X=(tau)   <error( X )>
	enddo
