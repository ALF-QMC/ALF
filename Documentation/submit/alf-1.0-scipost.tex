...
Of course the ALF package is not the first open source project that aims at the simulation of physical models from solid state physics and the most notable among thes projects is certainly the ALPS library. Other names that come to mind within the realm of the solid state physics
are QUEST, TRIQS, w2dynamics, iQist and Wien2K.
IQist, TRIQS and w2dynamics focus on approximate solutions via the CT-HYB algorithm within the DMFT approximation. Therefore they cannot cope with large scale lattice simulations in the sense that spatial collective fluctuations are truncated. 
Wien2K is representative for all DFT based software packages. Systems with large unit cells are certainly amenable to DFT  in the local density approximation. The sheer amount of predictions done with 
...
The QUEST project implements the same algorithm as the ALF project does but it is lacking in two major categories. It is currently restricted to the simple Hubbard model
and it does not allow to easily incorporate different models.
Furthermore,  it does not seem to be actively maintained anymore; the last publication dates from 2013.
An entirely different  approach is  the ALPS. It is actively maintained by an lively group of volunteers as well as paid staff and features a whole set 
of algorithms for physics. Using the DMRG algorithms they are able to simulate large 1D Systems but they lack the tools for 2D  fermonic  systems offered in the ALF.
...
The organoizational structure of our project is simple. Anybody can use the code
...
We think that it is beneficial to users that they can benfit from the experience of others
