...
the strategy of forming a product of QR-decompositions which was proven to be weakly backwards stable in \mycomment{Ref.} \cite{Bai2011}.
...
Due to a theorem by van der Sluis \cite{vanderSluis1969} we know that the choice in \mycomment{Eq.} \eqref{eq:diagnorm} is almost optimal among all diagonal matrices $\bm{D}$ from the space of diagonal matrices 
...
% $Auto_{S_{\hat{S}^{z}}}(t_{\textrm{QMC}})$ for the $z$ spin direction, 
% $Auto_{(S_{\hat{S}^{x}} + S_{\hat{S}^{y}})/2}(t_{\textrm{QMC}})$ for the $xy$ plane, and
% $Auto_{(S_{\hat{S}^{x}} + S_{\hat{S}^{y}}+ S_{\hat{S}^{z}})/3}(t_{\textrm{QMC}})$ for the total spin.
