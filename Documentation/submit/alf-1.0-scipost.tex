...
In Fig.~\ref{fig_autocorr} (left) we present the autocorrelation functions for the Spin-Spin-correlation at $(\pi,\pi)$ both in the $xy$-plane and the $z$ direction. The Hubbard model on a square lattice is $SU(2)$ symmetric. However, we chose the Hubbard-Stratonovic decoupling in the $M_z$ channel such that each configuration breaks the symmetry which is only restored on average. This model shows spontaneous symmetry by the formation of an anti-ferromagnet and the analyzed observable represent the square of the order parameter. Fig.~\ref{fig_autocorr} (left) clearly shows a very short autocorrelation time for the $xy$-plane whereas we detect a considerably longer time scale for the $z$-direction. This is a direct consequence of both the spontaneous symmetry breaking and the chose decoupling.
