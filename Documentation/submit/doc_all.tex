...
\end{equation}
To boil this down to more familiar terms from linear algebra we remark that we can recast this problem as the question of how to solve the linear system
\begin{equation}
...
\end{enumerate}
While this might look like quite some additional effort that has to be invested 
for every product it has to be noted that even with this stabilization scheme the algorithm preserves the complexity class of $\mathcal{O}(N^3)$.
...
\hspace*{2em} \textbf{If} \texttt{ntau} equals \texttt{1} \textbf{then}\\
\hspace*{3em} \textbf{Call} tau\_m\\
