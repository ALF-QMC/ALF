...
!--------------------------------------------------------------------
!> @author ALF-project
!> @brief Imaginary-time and Matsubara frequency Fourier transformations.
!> @details Implements fast algorithms for transforming between imaginary
!> time τ ∈ [0, β] and Matsubara frequencies ωₙ = (2n+1)π/β (fermions)
!> or 2πn/β (bosons).
!>
!> **Key Interfaces:**
!> - Matz_tau: Matsubara frequency → imaginary time (via Fourier series)
!> - Tau_Matz: Imaginary time → Matsubara (via MaxEnt analytical continuation)
!> - Variants: T0 (T=0), matrix (multi-orbital), stochastic
!>
!> **Tail Handling:** High-frequency components subtracted before summation
!> for numerical accuracy, then added back analytically.
!--------------------------------------------------------------------
Module  Fourier
