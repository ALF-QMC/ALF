...
!--------------------------------------------------------------------
    ...
    submodule (Hamiltonian_main) ham_Hubbard_SU2_smod
      ...
      type, extends(ham_base) :: ham_Hubbard_SU2
      contains
...
#endif
      end type ham_Hubbard_SU2

      !#PARAMETERS START# VAR_lattice
      Character (len=64) :: Model = 'Hubbard_SU2'  ! Value not relevant
      ...
      !#PARAMETERS START# VAR_Hubbard_SU2
    ...
    contains
      ...
      module Subroutine Ham_Alloc_hubbard_su2
        allocate(ham_Hubbard_su2::ham)
      end Subroutine Ham_Alloc_hubbard_su2
...
!--------------------------------------------------------------------
        Subroutine Ham_V
          ...
          Integer :: nf, I, I1, I2,  nc,  J, no,  N_ops,s 
          ...
          Do I = 1, Latt%N
            Do s = 1,3
               select case (s)
               case (1)
                  x = cos(n_quant(i,1))*sin(n_quant(i,2))
                  N_ops = N_ops + 1
                  Call OP_Make( Op_V(N_ops,N_FL),2)
                  Op_V(N_ops,N_FL)%P(1)   = invlist(I,1)
...
!-------------------------------------------------------------------
        subroutine Obser(GR,Phase,Ntau, Mc_step_weight)
          ...
          Integer :: I,J, imj, nf, dec, I1, J1, no_I, no_J,n, I_up, I_do
...
!-------------------------------------------------------------------
    ...
    end submodule ham_Hubbard_SU2_smod
