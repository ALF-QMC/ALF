...
!>    Public variables. Have to be set by user
      ...
      Type (Fields)        :: nsigma
      Complex (Kind=Kind(0.d0)), dimension(:), allocatable :: w_coeffi
      Integer              :: Ndim
      Integer              :: N_FL
      Integer              :: N_SUN
      Integer              :: Ltrot
...
!>    Privat variables
      ...
      real (Kind=Kind(0.d0)),private :: ham_T, Ham_chem, Ham_g, Ham_J,  Ham_K, Ham_h,  Ham_TZ2, Ham_U, Ham_lambda
...
!--------------------------------------------------------------------
      Subroutine Ham_Set
          ...
          CALL MPI_BCAST(l_hmc      ,1,   MPI_LOGICAL  , 0,Group_Comm,ierr)
          CALL MPI_BCAST(Ham_lambda ,1,   MPI_REAL8    , 0,Group_Comm,ierr)
          CALL MPI_BCAST(mdstep     ,1,   MPI_INTEGER  , 0,Group_Comm,ierr)
          CALL MPI_BCAST(dt         ,1,   MPI_REAL8    , 0,Group_Comm,ierr)
#endif
          ...
          If  ( Model == "Z2_Matter" )  Call Setup_Ising_action_and_field_list 
           ...
           If (Irank == 0 ) then
              ...
              elseif (Abs(Ham_TZ2) < Zero) then
                 ...
                 Write(50,*) 'lambda        : ', Ham_lambda 
              endif
              Write(50,*) 'l_hmc         : ', l_hmc
              If ( l_hmc ) then
                Write(50,*) 'dt            : ', dt
                Write(50,*) 'mdstep        : ', mdstep
              endif
           ...
           gamma_s = -0.5d0*log(tanh(dtau*Ham_g))

           if (l_hmc) call Ham_Akw_phi
          ...
          Do nc = 1, N_ops
             ...
             case (1 ) ! Z2_Gauge
                ...
                do nf = 1,N_FL
                   ...
                   Op_V(nc,nf)%g      = cmplx(dtau*Ham_TZ2,0.D0,kind(0.D0))
                   Op_V(nc,nf)%alpha  = cmplx(0d0,0.d0, kind(0.D0)) 
                   Op_V(nc,nf)%type   = 3
...
!>
!> @brief
!> Setting up the field quadratic action in momentum frequency basis
!-------------------------------------------------------------------
        Subroutine Ham_Akw_phi
          ...
          allocate(w_coeffi(ltrot))
          
          do i = 0, ltrot-1
              w_coeffi(i+1) = &
                  dcmplx( 0.25d0/(gamma_s*cos(2.d0*pi*dble(i)/dble(ltrot))+Ham_lambda*dtau), 0.d0 )
          enddo

        end Subroutine Ham_Akw_phi
...
!>
!> @brief
!> Deallocate the coeffiences array
!-------------------------------------------------------------------
        Subroutine Akw_phi_deallocate
        ...
        end Subroutine Akw_phi_deallocate
