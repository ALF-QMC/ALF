...
!> set of predefined Hamiltonians. They include the Hubbard and SU(N) U1_gauge models
...
!--------------------------------------------------------------------
    ...
    submodule (Hamiltonian_main) ham_U1_gauge_smod
      ...
      type, extends(ham_base) :: ham_U1_gauge
      contains
...
#endif
      end type ham_U1_gauge

      !#PARAMETERS START# VAR_lattice
      Character (len=64) :: Model = 'U1_gauge'  ! Value not relevant
    ...
    contains
      ...
      module Subroutine Ham_Alloc_U1_gauge
        allocate(ham_U1_gauge::ham)
      end Subroutine Ham_Alloc_U1_gauge
...
#include "Hamiltonian_U1_gauge_read_write_parameters.F90"
...
!--------------------------------------------------------------------
      Subroutine Ham_Set
          ...
          ! From dynamically generated file "Hamiltonian_U1_gauge_read_write_parameters.F90"
...
!--------------------------------------------------------------------
        Subroutine Ham_Vint
          ...
          !Use predefined hoppings to manage the bonds since the interaction of the U1_gauge model is exactly on the hopping bonds
          Select case (str_to_upper(Lattice_type))
          Case ("SQUARE")
          ...
          do nf = 1,N_FL
             ...
             Do n_f = 1, Bond_Matrix(1)%N_FAM
                Do l_f = 1, Bond_Matrix(1)%L_Fam(n_f)
                   ...
                   Op_V(nc,nf)%type=2
                   Op_V(nc,nf)%O_V_func => O_V_func
                   Call Op_set(Op_V(nc,nf))
                   Z =  cmplx(0.d0,ranf_wrap()*2.d0*3.1415927, Kind(0.d0))
                   Write(6,*) Op_V(nc,nf)%O_V_func(Z)
                enddo
             enddo
          enddo
...
!-------------------------------------------------------------------
    ...
    end submodule ham_U1_gauge_smod
