...
!>  If N_FL = 2 then N_SUN = 1 
!--------------------------------------------------------------------
      Complex (Kind=Kind(0.d0)) function Predefined_Obs_Cotunneling(I_c, I_f, J_c, J_f,  GT0,G0T,G00,GTT, N_SUN, N_FL,Spin) 
        ...
        elseif (N_FL == 2 )   then  !  This works only for N_SUN = 1 
           ...
           If (Present(Spin)) then
            select CASE (spin)
            CASE (1)
              Z = - 0.25*G1(1,3,2)*G1(2,2,1)*G1(3,1,2) +                                &
                  & G1(1,2,1)*(0.25*G1(2,3,1)*G1(3,1,1) - 0.5*G1(2,3,2)*G1(3,1,2)) +    &
                  & G1(2,3,1)*(-0.5*G1(1,2,2)*G1(3,1,2) +                               &
                  & G1(1,1,1)*(-0.25*G1(3,2,1) - 0.5*G1(3,2,2)) +                       &
                  & G1(1,1,2)*(0.25*G1(3,2,1) + 0.5*G1(3,2,2))) +                       &
                  & G1(1,3,1)*(-0.25*G1(2,2,1)*G1(3,1,1) - 1.*G1(2,2,2)*G1(3,1,2) +     &
                  & G1(2,1,1)*(0.25*G1(3,2,1) + 0.5*G1(3,2,2)) +                        &
                  & G1(2,1,2)*(0.5*G1(3,2,1) + 1.*G1(3,2,2))) +                         &
                  & (G1(1,2,1)*(-0.25*G1(2,1,1) - 0.5*G1(2,1,2)) +                      &
                  & (0.25*G1(1,1,1) - 0.25*G1(1,1,2))*G1(2,2,1))*G1(3,3,1) +            &
                  & (G1(1,2,1)*(0.25*G1(2,1,1) + 0.5*G1(2,1,2)) +                       &
                  & (-0.25*G1(1,1,1) + 0.25*G1(1,1,2))*G1(2,2,1))*G1(3,3,2)
            CASE (2)
               Z = - 0.25*G1(1,3,1)*G1(2,2,2)*G1(3,1,1) -                                       &
                  & 0.5*G1(1,2,2)*G1(2,3,1)*G1(3,1,1) - 0.5*G1(1,2,1)*G1(2,3,2)*G1(3,1,1) +     &
                  & 0.25*G1(1,2,2)*G1(2,3,2)*G1(3,1,2) + 0.5*G1(1,1,1)*G1(2,3,2)*G1(3,2,1) -    &
                  & 0.5*G1(1,1,2)*G1(2,3,2)*G1(3,2,1) +                                         &
                  & G1(1,3,2)*(-1.*G1(2,2,1)*G1(3,1,1) - 0.25*G1(2,2,2)*G1(3,1,2) +             &
                  & G1(2,1,2)*(0.5*G1(3,2,1) + 0.25*G1(3,2,2)) +                                &
                  & G1(2,1,1)*(1.*G1(3,2,1) + 0.5*G1(3,2,2))) +                                 &
                  & 0.25*G1(1,1,1)*G1(2,3,2)*G1(3,2,2) - 0.25*G1(1,1,2)*G1(2,3,2)*G1(3,2,2) +   &
                  & 0.5*G1(1,2,2)*G1(2,1,1)*G1(3,3,1) + 0.25*G1(1,2,2)*G1(2,1,2)*G1(3,3,1) +    &
                  & 0.25*G1(1,1,1)*G1(2,2,2)*G1(3,3,1) - 0.25*G1(1,1,2)*G1(2,2,2)*G1(3,3,1) -   &
                  & 0.5*G1(1,2,2)*G1(2,1,1)*G1(3,3,2) - 0.25*G1(1,2,2)*G1(2,1,2)*G1(3,3,2) -    &
                  & 0.25*G1(1,1,1)*G1(2,2,2)*G1(3,3,2) + 0.25*G1(1,1,2)*G1(2,2,2)*G1(3,3,2)    
            CASE DEFAULT
               write(error_unit, *) 'Invalid value for spin for calculation of spin resolved composite fermion. Spin = 1,2 only'
